...
module.exports = {
    ...
    passwordControl: async (req, res, next) => {

        const method = req.method
        const id = req.params?.id
        const { name, surname, email, password } = req.body

        console.log("user param id: ", id)
        ...
        if (method == 'POST' && name && surname && email && password) {
            ...
            const mailOption = { ...updateForUsers, to: userData.email }

            if (password !== passEncry(userData.password)) {
